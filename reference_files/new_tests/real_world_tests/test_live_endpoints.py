...
class LiveEndpointTester:
    ...
    def test_creativity_flow(self):
        """Test the creativity assessment flow"""
        ...
        if result["success"]:
            ...
            matched = [phrase for phrase in trigger_phrases if phrase in response_text]
            print(f"🔎 Matched phrases: {matched}")
            creativity_triggered = bool(matched)
            self.add_result(
                "Creativity Flow Trigger",
                creativity_triggered,
                "Creativity assessment triggered" if creativity_triggered else "Creativity assessment not triggered",
                result["duration"],
                result["data"]
            )
            if creativity_triggered:
                follow_up = self.send_message("A) I prefer to think through problems systematically")
                if follow_up["success"]:
                    follow_response = follow_up["data"].get("answer", "").lower()
                    print(f"🤖 Full AI Response (Follow-up):\n{follow_response}\n")
                    test_progression_phrases = [
                        "question", "next", "continue", "thanks", "option"
                    ]
                    print(f"🔍 Checking for progression phrases: {test_progression_phrases}")
                    matched_progression = [phrase for phrase in test_progression_phrases if phrase in follow_response]
                    print(f"🔎 Matched progression phrases: {matched_progression}")
                    test_progression = bool(matched_progression)
                    self.add_result(
                        "Creativity Test Progression",
                        test_progression,
                        "Creativity test progressed" if test_progression else "Creativity test didn't progress",
                        follow_up["duration"]
                    )
        else:
            print(f"❌ Request failed: {result['error']}")
            self.add_result("Creativity Flow Trigger", False, f"Request failed: {result['error']}")

    def test_project_overview_flow(self):
        """Test the project overview onboarding flow"""
        self.log_test_start("Project Overview Flow")
        # Complete the full intro flow first
        self.complete_intro_flow()
        # Now trigger the project onboarding
        result = self.send_message("I want to start a new creative project - a science fiction novel about AI.")
        project_trigger_phrases = [
            "topic 1 of 8", "project overview", "onboarding", "let's explore",
            "comprehensive understanding", "project description"
        ]
        if result["success"]:
            response_text = result["data"].get("answer", "").lower()
            print(f"🔍 Checking for project trigger phrases: {project_trigger_phrases}")
            matched = [phrase for phrase in project_trigger_phrases if phrase in response_text]
            print(f"🔎 Matched project trigger phrases: {matched}")
            project_triggered = bool(matched)
            self.add_result(
                "Project Overview Trigger",
                project_triggered,
                "Project onboarding triggered" if project_triggered else "Project onboarding not triggered",
                result["duration"],
                result["data"]
            )
            # Test progression through onboarding if triggered
            if project_triggered:
                follow_up = self.send_message("The story is about an AI that learns to paint.")
                if follow_up["success"]:
                    follow_response = follow_up["data"].get("answer", "").lower()
                    print(f"🤖 Full AI Response (Project Follow-up):\n{follow_response}\n")
        else:
            print(f"❌ Request failed: {result['error']}")
            self.add_result("Project Overview Trigger", False, f"Request failed: {result['error']}")

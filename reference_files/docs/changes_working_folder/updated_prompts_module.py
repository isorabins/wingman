#!/usr/bin/env python3
"""
Prompts Module - Updated for Claude API + Context Injection + Archetype Personalization

Contains prompts optimized for direct Claude API usage with automatic context injection
and personality adaptations based on user creativity archetypes.

Architecture Changes:
- Removed all search tool references (no more LangChain agents)
- Updated for context injection pattern (all user data pre-loaded)
- Optimized for Claude's prompt caching capabilities
- Focus on natural memory recall vs database searching
- Added archetype-based personality adaptations

Contains the main system prompt and other prompt templates for the Fridays at Four agent.
"""
...
# Usage example for SimpleClaudeClient integration
async def get_user_archetype(user_id: str, supabase_client=None) -> Optional[int]:
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            archetype_value = result.data[0].get('archetype')
            # Convert string to int if needed
            if isinstance(archetype_value, str):
                return int(archetype_value)
            return archetype_value
        
        logger.info(f"No archetype found for user {user_id}, using default")
        return None
        
    except Exception as e:
        logger.error(f"Error looking up archetype for user {user_id}: {e}")
        return None
...
async def send_personalized_message(
    ...
    **kwargs
    ...
    """
    Send a message with archetype-specific style guidance to Claude
    
    Args:
        messages: List of conversation messages
        user_id: User's ID to look up archetype
        context: User context for injection
    ...
    # Get user's archetype from database
    archetype = await get_user_archetype(user_id, supabase_client)
    
    # Get personalized prompt with consistent temperature
    system_prompt = get_personalized_prompt(main_prompt, archetype, context)
    temperature = get_archetype_temperature()  # Consistent 0.6 for all users
    ...
    # Send to Claude with consistent temperature but personalized style guidance
    config = Config()
    ...
    return await client.send_message(
        all_messages,
        temperature=temperature,  # Consistent 0.6 for all archetypes
        **kwargs
    )
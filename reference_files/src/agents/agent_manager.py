...
- IntroAgent for mandatory intro conversations
...
class AgentManager:
    ...
    def _get_agent(self, user_id: str, agent_type: str):
        ...
        if agent_key not in self._agents:
            if agent_type == "intro":
                self._agents[agent_key] = IntroAgent(self.supabase, user_id)
            elif agent_type == "creativity_test":
    ...
    async def determine_agent_needed(self, user_id: str, user_message: str, thread_id: str) -> Tuple[str, str]:
        ...
        """
        try:
            # FIRST: Check if intro is needed (mandatory for new users)
            intro_agent = self._get_agent(user_id, "intro")
            intro_complete = await intro_agent.is_complete()
            
            if not intro_complete:
                return "intro", "Mandatory intro conversation required for new users"
            
            # SECOND: Check prerequisites - creativity test must come before project overview
